...
let (json_of_tests_coverage: tests_coverage -> J.t) = fun cov ->
  J.Object (cov |> Ls.map (fun (cover_file, tests_score) ->
    cover_file,
    J.Array (tests_score |> Ls.map (fun (test_file, score) ->
...
let (tests_coverage_of_json: J.t -> tests_coverage) = fun j ->
  match j with
  | J.Object xs ->
      xs |> Ls.map (fun (cover_file, tests_score) ->
        cover_file,
        match tests_score with
        | J.Array zs ->
            zs |> Ls.map (fun test_file_score_pair ->
...
let (json_of_lines_coverage: lines_coverage -> J.t) = fun cov ->
  J.Object (cov |> Ls.map (fun (file, cover) ->
    file,
    J.Object ([
      (* I use short fieldnames to avoid generating a huge JSON file.
      *)
      "cov", J.Array (cover.covered_sites |> Ls.map (fun l -> J.Int l));
      "all", J.Array (cover.all_sites |> Ls.map (fun l -> J.Int l));
    ])
  ))
      ...
      xs |> Ls.map (fun (file, cover) ->
        ...
        | J.Object ([
          ...
          "all", J.Array call_sites;
            ...
            {
              covered_sites =
                covered_lines |> Ls.map (function
                  | J.Int l -> l
                  | _ -> failwith "Bad json, files_coverage_of_json"
                );
              all_sites =
                call_sites |> Ls.map (function
                  | J.Int l -> l
                  | _ -> failwith "Bad json, files_coverage_of_json"
                );
            }
