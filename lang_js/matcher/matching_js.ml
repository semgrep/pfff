...
*)
  ...
  (* pre: both 'a' and 'b' contains only regular JS code. There is no
  ...
  let equal_ast_binded_code a b =
    ...
    | Ast.Expr _, Ast.Expr _ 
      ->
         ...
         *)
        let a = Lib_analyze_js.abstract_position_info_any a in
        let b = Lib_analyze_js.abstract_position_info_any b in
        a =*= b
        
    | _, _ -> 
        false

  let check_and_add_metavar_binding((mvar:Metavars_js.mvar), valu) = fun tin ->
  ...
  let (envf: (Metavars_js.mvar Ast_js.wrap, Ast_js.any) matcher) =
  ...
  let (envf2: (Metavars_js.mvar Ast_js.wrap, Ast_js.any * Ast_js.any) matcher) =
...
module MATCH  = Ast_js_vs_js.JS_VS_JS (XMATCH)
