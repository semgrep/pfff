...
and simplePattern in_ : pattern =
  in_ |> with_logging "simplePattern" (fun () ->
    ...
    *)
    (* matthew: [val - = expr] and [case - => expr] are valid Scala, so we check that - is not the end of the pattern *)
    | MINUS ii when lookingAhead (fun in_ -> match in_.token with EQUALS _ | ARROW _ -> false | _ -> true) in_ ->
        nextToken in_;
